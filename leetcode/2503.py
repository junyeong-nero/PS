...
class Solution:
    def maxPoints(self, grid, queries):
        ...
        sorted_queries = sorted(
            [(val, idx) for idx, val in enumerate(queries)]
        )  # Sort queries by value, keeping track of original indices
        result = [0] * len(queries)  # Initialize the result array with zeros

        heap = (
            PriorityQueue()
        )  # Initialize a priority queue to store cells to visit, ordered by their values
        visited = [
        ...
        for (
            query_val,
            query_idx,
        ) in sorted_queries:  # Iterate through the sorted queries
            while (
                not heap.empty() and heap.queue[0][0] < query_val
            ):  # While the heap is not empty and the smallest cell value is less than the current query value
                _, row, col = (
                    heap.get()
                )  # Get the cell with the smallest value from the heap
                ...
                for dr, dc in DIRECTIONS:  # Iterate through the possible directions
                    ...
                    if (
                        0 <= nr < rows and 0 <= nc < cols and not visited[nr][nc]
                    ):  # If the neighbor is within the grid bounds and not visited
                        heap.put((grid[nr][nc], nr, nc))  # Add the neighbor to the heap
                        visited[nr][nc] = True  # Mark the neighbor as visited

            result[query_idx] = (
                points  # Store the current points count in the result array at the original query index
            )
